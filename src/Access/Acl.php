...
class Acl
{
    ...
    public function isAllowed($dispatcher)
    {
        ...
        $componentSlug = $this->slugifyResource(
        ...
        if ($this->config->access->endpoint === AclEnum::CONFIG_ENDPOINT_TYPE) {
            ...
            $dataAction   = $annotations->getMethod($controller, $actionSlug . 'Action');
            ...
            try {
                ...
                $domainResource = $dataResource->get('domain')->getArguments()[0];
                $componentSlug  = $this->slugifyResource($moduleName, $domainResource);
            }
            catch (\Throwable $t) {
            ...
            try {
                $typeAccess = $dataAction->get('accessType')->getArguments()[0];
                $actionSlug = $typeAccess;
            }
            catch (\Throwable $t) {
                throw new DoesntAnnotation("This Action " . $t->getMessage());
            }
     ...
     * @return array
     */
    public function slugifyResource($moduleName, $resource): string
    {
        return strtolower(
